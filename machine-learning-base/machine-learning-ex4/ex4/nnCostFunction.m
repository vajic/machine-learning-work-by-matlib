...
Theta1_1 = Theta1(:,2:size(Theta1,2));
Theta2_2 = Theta2(:,2:size(Theta2,2));
J = sum(sum((-yt).*log(hx2)-(1-yt).*log(1-hx2)))/m +lambda*(sum(sum(Theta1_1.^2)) + sum(sum(Theta2_2.^2)))/(2*m);


% Theta1_grad = X'*log(hx1-y)/m;
% Theta2_grad = X'*log(hx2-y)/m;
...
for i=1:m
   ...
   % Z3 = tmp2(i,:);
    ...
    Err3 = A3 - Out;
    Err2 = Theta2(:,2:end)'*Err3.*sigmoidGradient(Z2);
